...
def remove_punct(text):
    ...
    'goSouTh'
    """
     a = str.maketrans({key: None for key in string.punctuation})
     no_punct = text.translate(a)
     return no_punct
...
def menu(exits):
    ...
    while True:
        ...
        if is_valid_exit(exits, normalised_input):
            #display name of chosen exit
           #MOVE PLAYER IF EXIT IS VALID
          return normalised_input
        else:
            print("You entered an invalid exit, try again")
            menu(exits)
